...
# Create one record
try:
    new_user = User(name="Harsh", age = 21)
...
# Update
try:
    updated_details = session.query(User).filter_by(name="Harsh").first()
    if updated_details:
...
# Delete
try:
    deleted_details = session.query(User).filter_by(name="Yash").first()
    if deleted_details:
